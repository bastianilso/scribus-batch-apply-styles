...
def main():
    ...
    try:
        ...
        style = valueDialog('Apply Paragraph Style To Selection', 'Choose paragraph style by typing its exact name below or copy from the name pre-filled text.', ' '.join(getCharStyles()))
        
        content = []       
        # getParagraphStyle
        for item in items:
                contents = scribus.getAllText(item)
                setCharacterStyle(style, item)
